...
<section class="min-w-full bg-green-600 pt-16">
    <div class="max-w-screen-xl mx-auto">
        <h1 class="pt-10 pb-5 px-16 font-semibold md:text-3xl sm:text-2xl text-xl text-gray-100">{{ $profile->category->name }}</h1>
    </div>
</section>
<section class="w-full bg-gray-100 overflow-hidden">
    <div class="grid grid-cols-10 max-w-screen-xl mx-auto lg:space-x-3 lg:px-10 px-3">
        <div class="lg:col-span-7 col-span-10 p-5 shadow-md rounded-md mx-auto bg-white mt-3 lg:mb-10">
            <h1 class="md:text-3xl sm:text-2xl text-lg font-semibold text-gray-800">{{ $profile->title }}</h1>
            <div class="text-base font-poppins text-slate-800 leading-7 w-full space-y-3">{!! $profile->body !!}</div>
        </div>
        <div class="lg:col-span-3 col-span-10">
            <div class="w-full max-w-md p-4 bg-white border border-gray-200 rounded-md my-3 shadow-md sm:p-8 dark:bg-gray-800 dark:border-gray-700">
                ...
                <ul>
                    ...
                    <li class="mb-1">
                        <a href="{{ route('profile.show', ['title' => $prof->title]) }}" class="{{ $profile->id == $prof->id ? 'text-green-600' : '' }} hover:text-green-600 duration-300">{{ $prof->title }}</a>
                    </li>
                    @endforeach
                </ul>
            </div>
